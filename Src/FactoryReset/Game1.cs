...
namespace GameManager
{
    public class Game1 : Game
    {
        ...
        private readonly ConcurrentQueue<Action<Game1>> ActionQueue 
            = new ConcurrentQueue<Action<Game1>>();
        ...
        private void UpdateLoadName(object identifier)
        {
            ...
            {
                ((LoadScreen)RealActiveWindow).LevelName = " " 
                    + LevelContent.Read(identifier, true).name;
            }
            AdvanceLoad();
        }
        ...
        public void Resize(int width, int height)
        {
            Transforms.ProjectionMatrix = 
                Matrix.CreateOrthographicOffCenter(0, width, 0, height, -10, 10);
