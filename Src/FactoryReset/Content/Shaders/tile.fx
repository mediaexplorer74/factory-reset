...
float2 tilemapSize; // width,height in tiles of the tilemap texture ((passed from engine)
...
{
     if (input.tileValid < 0.0f)//0.5f)
        return float4(0, 0, 0, 0);

     // Sample tilemap in pixel shader (allowed)
     float2 tileUV = (input.tileIndexF + 0.5f) / tilemapSize;  // / tilemapSize;
     ...
     float2 sampleUV = samplePixel / tilesetSize; // / tilesetSize;
