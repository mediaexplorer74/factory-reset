...
namespace GameManager
{
    public class TextEngine
    {
            ...
            bool lineWrapping = false, float textwidth = 0)
        {
            ...
            switch(horizontal)
            {
                ...
                default: throw new ArgumentException(
                    String.Format("{0} is not a valid horizontal orientation.", horizontal));
            }
            switch(vertical)
            {
                ...
                default: throw new ArgumentException(
                    String.Format("{0} is not a valid vertical orientation.", vertical));
            }
            
            QueuedText.Add(new Text(text, pos, color, font, scale, -1));
        }
        
        public void QueueText(string text, Vector2 position, 
            string fontName=DefaultFont, float sizePx=DefaultSize, 
            Orientation horizontal=Orientation.Left, Orientation vertical=Orientation.Bottom)
        {
        ...
        public void QueueText(string text, Vector2 position, float sizePx, 
            Orientation horizontal=Orientation.Left, Orientation vertical=Orientation.Bottom){
            QueueText(text, position, Color.Black, DefaultFont, sizePx, horizontal, vertical);
        }
        
        public void QueueText(string text, Vector2 position, float sizePx, Color color,
            Orientation horizontal=Orientation.Left, Orientation vertical=Orientation.Bottom){
        ...
        public void QueueButton(Button button, Vector2 position, Color color)
        {
            ...
            QueueText(text, position, color, "promptfont", DefaultSize, Orientation.Center, 
                Orientation.Center);
        }
